...
def _num_identify_initial_pivot(X, up_thresh, down_thresh):
    """Quickly identify the X[0] as a peak or valley."""
    x_0 = _to_float(X[0])
    max_x = x_0
    max_t = 0
    ...
    t_n = len(X) - 1
    return VALLEY if x_0 < _to_float(X[t_n]) else PEAK
...
def num_peak_valley_pivots(X, up_thresh, down_thresh):
    ...
    initial_pivot = _num_identify_initial_pivot(X, up_thresh, down_thresh)
